...
def create_threshold_network(n):
    ...
    for name in n.nodes():
        ...
        if 'state' not in n.node[name] and n.node[name]['type']=='variable':
            state_str=name.replace(n.node[name]['variable'],"",1) #remove variable name
...
def BFS_threshold(n,seed,input_type='steady',time_limit=10,conversion=True,pinned=set([])):
    ...
    while time<=max(time_steps): #allows for possibility of time jumps, i.e. updates further ahead than one time step
        ...
        if input_type=='steady': #don't allow contradictions; ADDED due to broken DCM function
            vars_seen={n.node[x]['variable']:n.node[x]['state'] for x in active_step if n.node[x]['type']=='variable'}
            contradictory_nodes.update({node for node in n if 
                        n.node[node]['type']=='variable' and n.node[node]['variable'] in vars_seen and n.node[node]['state']!=vars_seen[n.node[node]['variable']]})
            #print(vars_seen,contradictory_nodes)        
